...
import { Box, Chip, Container, Grid, Hidden, TextField, MenuItem, withStyles } from '@material-ui/core'
                            ...
                            <Grid item xs={12} sm={2}>
                                <Hidden xsDown>
                                    <Box className={classes.productsFilter}>
                                        ...
                                        {props.filter.map((filter: any, index: any) =>
                                            <Chip key={index} label={filter} color='primary' variant="outlined" onDelete={() => props.removeFilter(filter)} className={classes.chip} />
                                        )}
                                    </Box>
                                ...
                                <Box>
                                    ...
                                    <Box className={classes.catalog}>
                                        <Box className={classes.catalogList}>
                                            {props.goods.length > 0 ? props.goods.map((item: any) => <Good good={item} key={item._id} />) : <SiteMessage text="К сожалению по выбранному запросу товаров нет в наличии" />}
                                        </Box>
                                    </Box>
                                </Box>
